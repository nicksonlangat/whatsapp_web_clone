...
const conversations = ref([
  {
    content: 'Hey, did you manage to get insurance for your car? I gave the agent your number.',
    time: '13:52',
    type: 'received'
  },
  {
    content: "I have called the guy and he's working on it. Let's wait.",
    time: '13:55',
    type: 'sent',
    is_read: true
  },
  {
    content: 'Yeah it is only right that he calls so that you can arrange. He will have to draft a legal agreement.',
    time: '13:58',
    type: 'received'
  },
  {
...
const chats = ref([
  {
    ...
    name: 'Nick Adams',
    time: '05/01/2024',
    content: 'Hey, did you manage to get insurance for your car?'
  },
  {
    image: 'https://flowbite.com/docs/images/people/profile-picture-2.jpg',
    name: 'Alice Oryx',
    time: '05/01/2024',
    content: 'Wanna go out and grab fries later on?'
  },
  {
    image: 'https://flowbite.com/docs/images/people/profile-picture-3.jpg',
    name: 'Sasha Trex',
    time: '05/01/2024',
    content: 'Yeah, they said they would stop by tomorrow.'
  },
  {
...
<template>
  <main class="h-screen font-base flex flex-col overflow-hidden">
    <!-- Main content -->
    <div class="flex-1 divide-x divide-[#222d34] flex overflow-hidden">
      <!-- Fixed sidebar -->
      <div class="w-96 flex-col flex">
        <div class="border-b border-[#222d34] bg-[#1f2c33] py-2">
          <div class="p-3 flex justify-between">
            <img src="./assets/images/1.png" class="h-10 w-10 rounded-full object-cover" alt="" />
            <div class="flex gap-5 text-[#aebac1] cursor-pointer items-center">
              <span>
                <svg
                  viewBox="0 0 24 24"
                  height="24"
        ...
        <div class="mt-2">
          <div class="p-3 text-[#aebac1] relative">
            <input
              type="text"
              class="bg-[#1f2c33] w-full py-2 focus:outline-none focus:ring-0 pl-10 rounded-md"
              placeholder="Search"
            />
            <svg
              viewBox="0 0 24 24"
        ...
        <div class="p-3 text-[#aebac1] text-sm flex gap-2">
          <button class="bg-[#0b332c] text-[#05a884] py-1.5 px-4 rounded-full">All</button>
          <button class="bg-[#1f2c33] text-[#7e8e97] py-1.5 px-4 rounded-full">Unread</button>
          <button class="bg-[#1f2c33] text-[#7e8e97] py-1.5 px-4 rounded-full">Groups</button>
        </div>
        <div class="border-b border-[#222d34]"></div>

        <!-- Scrollable container -->
        <div class="px-2 py-4 flex-1 overflow-y-auto">
          <!-- Your content -->
          <div class="flex text-[#80919a] flex-col gap-5">
            <div v-for="chat in chats" :key="chat" class="flex gap-2 items-center">
              <img :src="chat.image" class="h-12 rounded-full object-cover w-12" alt="" />
              <div class="w-full border-b border-[#222d34]">
                <div class="flex justify-between">
                  <p class="text-[#e9edef] text-sm">{{ chat.name }}</p>
                  <p class="text-sm">{{ chat.time }}</p>
                </div>
                <div class="text-[#d1d7db] text-xs mt-1">
                  {{ chat.content }}
                </div>
                <div class="h-2"></div>
              </div>
      ...
      <div class="flex-1 flex-col flex">
        <div class="text-[#e9edef] py-1.5 bg-[#1f2c33]">
        ...
        <div
          ...
          "
          class="px-10 py-4 text-[#e9edef] mix-blend-dargken bg-[#0c1317] flex-1 overflow-y-auto"
        >
          <!-- Your content -->
        
          <div class="flex w-full text-sm flex-col gap-5">
            <div v-for="convo in conversations" :key="convo">
              <div v-if="convo.type == 'received'" class="flex  justify-start ">
                <div class="bg-[#1f2c33] rounded-md w-2/3">
                  <div class="p-3 w-full">
                    <p>{{ convo.content }}</p>
                    <div class="flex text-[#a5aaac] justify-end">
                      {{ convo.time }}
                    </div>
                  </div>
                </div>
              </div>
              <div v-if="convo.type == 'sent'" class="flex  justify-end ">
                <div class="bg-[#015d4b] rounded-md w-2/3">
                  <div class="p-3 w-full">
                    <p>{{ convo.content }}</p>
                    <div class="flex text-[#a5aaac] items-center gap-2 justify-end">
                      {{ convo.time }}
                      <svg viewBox="0 0 16 11" height="11" width="16" preserveAspectRatio="xMidYMid meet" class="text-[#4eb6df]" fill="none"><title>msg-dblcheck</title><path d="M11.0714 0.652832C10.991 0.585124 10.8894 0.55127 10.7667 0.55127C10.6186 0.55127 10.4916 0.610514 10.3858 0.729004L4.19688 8.36523L1.79112 6.09277C1.7488 6.04622 1.69802 6.01025 1.63877 5.98486C1.57953 5.95947 1.51817 5.94678 1.45469 5.94678C1.32351 5.94678 1.20925 5.99544 1.11192 6.09277L0.800883 6.40381C0.707784 6.49268 0.661235 6.60482 0.661235 6.74023C0.661235 6.87565 0.707784 6.98991 0.800883 7.08301L3.79698 10.0791C3.94509 10.2145 4.11224 10.2822 4.29844 10.2822C4.40424 10.2822 4.5058 10.259 4.60313 10.2124C4.70046 10.1659 4.78086 10.1003 4.84434 10.0156L11.4903 1.59863C11.5623 1.5013 11.5982 1.40186 11.5982 1.30029C11.5982 1.14372 11.5348 1.01888 11.4078 0.925781L11.0714 0.652832ZM8.6212 8.32715C8.43077 8.20866 8.2488 8.09017 8.0753 7.97168C7.99489 7.89128 7.8891 7.85107 7.75791 7.85107C7.6098 7.85107 7.4892 7.90397 7.3961 8.00977L7.10411 8.33984C7.01947 8.43717 6.97715 8.54508 6.97715 8.66357C6.97715 8.79476 7.0237 8.90902 7.1168 9.00635L8.1959 10.0791C8.33132 10.2145 8.49636 10.2822 8.69102 10.2822C8.79681 10.2822 8.89838 10.259 8.99571 10.2124C9.09304 10.1659 9.17556 10.1003 9.24327 10.0156L15.8639 1.62402C15.9358 1.53939 15.9718 1.43994 15.9718 1.32568C15.9718 1.1818 15.9125 1.05697 15.794 0.951172L15.4386 0.678223C15.3582 0.610514 15.2587 0.57666 15.1402 0.57666C14.9964 0.57666 14.8715 0.635905 14.7657 0.754395L8.6212 8.32715Z" fill="currentColor"></path></svg>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="bg-[#1f2c33] text-[#7e8e97]">
          <div class="flex p-5 gap-10 items-center w-full">
            ...
            <input
              type="text"
              class="bg-[#2a3942] w-full py-2 focus:outline-none focus:ring-0 pl-5 rounded-md"
              placeholder="Search"
            />
            <span>
              <svg
